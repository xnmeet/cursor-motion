...
// Build a custom spring config from response + dampingFraction. Useful
// when callers want to expose a SPRING knob like the original lab.
export function buildSpringConfig({
  response = 1.4,
  dampingFraction = 0.9,
  dt = 1 / 240,
  idleVelocityThreshold = 28800,
  closeEnoughProgressThreshold = 1,
  closeEnoughDistanceThreshold = 0.01,
} = {}) {
  const rawStiffness = response > 0 ? Math.pow(TWO_PI / response, 2) : Infinity;
...
// we collapse to the last 1/60s rather than catching up frame-by-frame. Under
// normal rAF cadence the gap is < 17ms and the clamp never triggers.
export function advanceTo(current, target, state, targetTime, config = OFFICIAL_SPRING) {
  ...
  // Cap iterations as a safety net for adversarial inputs.
  let safety = 0;
...
export function isCloseEnough(progress, target = 1, config = OFFICIAL_SPRING) {
  return (
    progress >= config.closeEnoughProgressThreshold &&
