...
export const OFFICIAL_SPRING: SpringConfig = (() => {
  const response = 1.4;
...
export function buildSpringConfig({
  ...
  closeEnoughProgressThreshold = 1,
  closeEnoughDistanceThreshold = 0.01,
}: Partial<SpringConfig> = {}): SpringConfig {
  const rawStiffness = response > 0 ? Math.pow(TWO_PI / response, 2) : Infinity;
  const stiffness = Math.min(rawStiffness, idleVelocityThreshold);
  const drag = 2 * dampingFraction * Math.sqrt(stiffness);
  return {
...
export function computeCloseEnoughTime(config: SpringConfig = OFFICIAL_SPRING): number {
  let current = 0;
...
export const DEFAULT_CLOSE_ENOUGH_TIME: number = computeCloseEnoughTime(OFFICIAL_SPRING);
