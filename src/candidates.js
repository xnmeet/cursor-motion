...
// orbit), then pick the one with the lowest score. Compared to the
// raw "20 candidates from the binary tableA/tableB" path, this generator
// gives a more natural single-sided C-shape on cross-axis moves.
...
// Description of one candidate family. Each row tweaks the path builder
// inputs (reach, normal bias, flow) and provides a small score bias so
// the chooser can prefer e.g. straight paths when turn-demand is low.
const DESCRIPTORS = [
...
function makePathFromDescriptor({ start, end, descriptor, startForward, endForward, params }) {
  ...
  const startHandle = params.startHandle ?? DEFAULT_START_HANDLE;
  const endHandle = params.endHandle ?? DEFAULT_END_HANDLE;
  const arcSize = params.arcSize ?? DEFAULT_ARC_SIZE;
  const arcFlow = params.arcFlow ?? DEFAULT_ARC_FLOW;
...
export function makeCandidates({
  ...
  return DESCRIPTORS.map((descriptor) => {
    const path = makePathFromDescriptor({
      start, end, descriptor, startForward: sf, endForward: ef, params,
    });
