...
export class DomCursorRenderer {
    constructor({ container = document.body, zIndex = 9999, showTrail = false, showCandidates = false, glyphHTML = DEFAULT_GLYPH_SVG, size = 48, } = {}) {
        this._trailPoints = [];
        ...
        this.onUpdate = (state) => {
            ...
            if (this.showTrail) {
                ...
                if (this._trailPoints.length > 200)
    ...
    setShowTrail(flag) {
        ...
        if (!flag)
            this._trailPoints = [];
        if (!flag && !this.showCandidates)
            this._clearSvg();
    }
    setShowCandidates(flag) {
        this.showCandidates = !!flag;
        if (!flag && !this.showTrail)
            this._clearSvg();
    }
    destroy() {
        this.root.remove();
...
//# sourceMappingURL=renderer-dom.js.map